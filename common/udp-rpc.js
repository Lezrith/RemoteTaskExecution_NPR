...
function sendMessage(address, payload, callback) {
	...
	if(payload.length > 494) {
		...
		for(i = 0; i < payload.length; i += 494) {
			var newPayload = Buffer.from(payload.slice(i, i + 493));
...
function receiveRpc(message, info) {
	...
	const flagId = header.flagid & ~128;
	if(!this.packets[source][flagId]) {
		this.packets[source][flagId] = [];
	}
	this.packets[source][flagId].push({
		header: header,
		body: body
	});
...
function attemptProcessing(source, id) {
	...
	const flagId = id & ~128;
	var packetArray = this.packets[source][flagId];
